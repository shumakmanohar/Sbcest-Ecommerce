	...
	return (
		<Link href={`/products/${product?.id}`}>
			<div className="relative flex flex-col bg-white  z-30 p-5 h-[460px] min-h-[460px]  rounded-xl transform transition-transform duration-500 hover:scale-105 cursor-pointer">
				...
				<p className="self-start mb-2 mt-2 italic rounded-full text-xs text-white bg-black rounded-full px-2">
					{product?.category.name}
				</p>
				<p className=" text-xs my-2 line-clamp-1 leading-[2]">{product?.description}</p>
